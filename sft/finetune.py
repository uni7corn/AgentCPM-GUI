...
class DataArguments:
    ...
    max_line_res: Optional[int] = field(default=1120)
...
def make_supervised_data_module(
    ...
    def load(path):
        if not path.endswith(('.json', '.jsonl')):
            raise ValueError('need .json or .jsonl')
        with open(path, encoding='utf-8') as f:
            return json.load(f) if path.endswith('.json') else [json.loads(l) for l in f if l.strip()]
    train_json = load(data_args.data_path)
    ...
    train_dataset = dataset_cls(
        ...
        max_line_res=data_args.max_line_res
    )

    if data_args.eval_data_path:
        eval_json = load(data_args.eval_data_path)
...
def train():
    ...
    transform_func = build_transform(data_args)
